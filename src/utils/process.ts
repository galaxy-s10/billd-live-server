...
export const mp4PushRtmp = (data: {
  ...
  const cmd = `ffmpeg -threads 1 -readrate 1 -f concat -safe 0 -i '${data.listTxt}' -vcodec copy -acodec copy -r 20 -f flv '${data.rtmpUrl}?token=${data.token}'`;
  // const cmd = `ffmpeg -threads 1 -readrate 1 -f concat -safe 0 -i '${data.listTxt}' -vcodec h264 -acodec aac -r 20 -f flv '${data.rtmpUrl}?token=${data.token}'`;
